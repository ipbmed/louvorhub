-- Slug editável + validade do link + listagem pública de eventos compartilhados

alter table public.events
  add column if not exists share_expires_at timestamptz;

-- Disponibilidade do slug (exclui o próprio evento)
create or replace function public.is_event_share_code_available(
  p_share_code text,
...
declare
  v_code text;
begin
  v_code := lower(trim(coalesce(p_share_code, '')));
  ...
  return not exists (
    ...
    where lower(e.share_code) = v_code
      and (p_exclude_event_id is null or e.id <> p_exclude_event_id)
  );
end;
$$;

revoke all on function public.is_event_share_code_available(text, uuid) from public;
grant execute on function public.is_event_share_code_available(text, uuid) to authenticated;

-- Lista eventos públicos ativos (não expirados), a partir de ontem
create or replace function public.list_public_shared_events()
returns jsonb
language sql
security definer
set search_path = public
as $$
  ...
  from public.events e
  where e.share_enabled = true
    and (e.share_expires_at is null or e.share_expires_at > now())
    and e.service_date >= (current_date - 1);
$$;

revoke all on function public.list_public_shared_events() from public;
grant execute on function public.list_public_shared_events() to anon, authenticated;

-- RPC pública com validade e slug customizável (mín. 3 chars)
...
begin
  ...
  return jsonb_build_object(
    'event', jsonb_build_object(
      ...
      'share_expires_at', v_event.share_expires_at,
